...
def generate_tfidf_vector(texts, data_size):
    ...
    for i, seq in enumerate(padded_sequences):
        ...
        for word, freq in word_freq.items():
            ...
            idf_value = np.log((data_size + 1) / (1 + sum([1 for s in all_sequences if word in s])))
...
def get_recommendations_from_search_terms(search_terms, data, top_n=5):
    ...
    aggregated_search_vector = tf.reduce_mean(normalized_search_vectors, axis=0, keepdims=True)
    cosine_similarities = tf.matmul(aggregated_search_vector, tf.transpose(normalized_item_vectors)).numpy().flatten()
